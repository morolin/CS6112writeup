...
This semester, we took the course CS6112, and applied the principles from that course, along with conventional PL theory reasoning to the Communicating Hardware Processes language used by the Asynchronous VLSI group at Cornell. 
...
However, we wanted to reason about programs in the CHP language, and do so in a way consistent with the calculi that we had been learning about in class.  Therefore, we built a labeled transition system in the CHP language, and discussed how it related to the HSE operational semantics that we had defined.
...
\item \textbf{Assignment}: $x := E$.  This statement means ``assign the value
